                ...
                <table   class="table table-bordered table-hover" id="tablajson">
                  ...
                  <tbody>
                   ...
                   $idcre=$_POST['idcre'];
                      ...
                      $total=0;
                      $p = new Creditos();
                       $result=$p->get_ventascredito($idcre);
                       
                        while($lista=$result->fetch_array()){  
                        $total=$total+ $lista['VEN_N_TOTAL'];  
                        
                        $date = date_create($lista['VEN_F_FECHAVENTA']);
                           ?>
                           <tr >
                             <td><?=$lista['COD_DOC']?></td>                      
                             <td><?=$lista['VEN_C_NOMBRECLIENTE']?></td>
                             <td><?php echo  "S/. ".$lista['VEN_N_TOTAL'] ?></td>
                              <td><?=date_format($date, 'd/m/Y H:i:s')?></td>
                           
                             <td><button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModal" data-id="<?=$lista['VEN_C_CODIGO']?>" data-nom="<?=$_POST['cliente']?>"  data-toggle="tooltip" title="Detalle"><i class="fa fa-mail-forward"></button></td>
                           </tr>
                           <?php } ?>
                  </tbody>
                  <tfoot>
