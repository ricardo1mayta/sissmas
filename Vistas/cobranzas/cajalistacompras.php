  ...
  auxfila=fila;
   fila=f;
    var user=$('#usco').val();
      ...
      var url="../Vistas/cobranzas/detallecompras.php";
...
var s=<?=$usuario['SED_C_CODIGO']?>;
      ...
      var url="../Vistas/cobranzas/comprassede.php";
...
function credito(t){
  var fv=$("#fechavence").val();
  var t=$("#idventaccc").val();
swal({
  title: "Atención!!",
  text: "Crear Credito?",
  type: "warning",
    ...
    <div class="modal-content " > 
      <div class="modal-body" >
          <div class="form-group ">
                ...
                <input type="date" id="fechavence" name="fechavence" class="form-control">
                </div>
              <div class="modal-footer">
              <input id="idventaccc"  type="hidden" name="idventaccc">
...
$('#acredito').on('show.bs.modal', function (event) {
  ...
  var modal = $(this)
  modal.find('.modal-title').text('Abonar a: ' + nombre)
  modal.find('input[name="idventaccc"]').val(id)
  modal.find('input[name="nombre"]').val(nombre)
  $('#tipopagos').prop('selectedIndex',0);
