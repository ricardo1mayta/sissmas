 ...
 $result=$p->get_ventastipo($sede,$t);
  ...
  while($lista=$result->fetch_array()){                        
     ...
     <tr id="sell<?=$lista[0]?>" >
       ...
       <td><button type="button" class="btn btn-primary btn-xs" onclick="imprimir('<?=md5($lista[0])?>','<?=$lista[5]?>')"><i class="fa fa-mail-forward">Imprimir</button></td>
       
       <td onclick="fin(<?=$lista[0]?>)"><i class="fa fa-ban"  data-toggle="tooltip" title="Anular"></i></td>
